...
const verifyState = [false, false];

const screens = [
  { id: 'concept', phase: 3, label: 'F3 · Vista y cuerpo' },
  { id: 'names', phase: 3, label: 'F3 · Nombres' },
  { id: 'families', phase: 3, label: 'F3 · Familias' },
  { id: 'quick', phase: 3, label: 'F3 · Verificación rápida' },
  { id: 'audit-intro', phase: 4, label: 'F4 · Instrucciones' },
  { id: 'audit', phase: 4, caseIndex: 0, label: 'F4 · Caso 1 de 3' },
  { id: 'audit', phase: 4, caseIndex: 1, label: 'F4 · Caso 2 de 3' },
  { id: 'audit', phase: 4, caseIndex: 2, label: 'F4 · Caso 3 de 3' },
  { id: 'verify', phase: 4, verifyIndex: 0, label: 'F4 · Verificación 1 de 2' },
  { id: 'verify', phase: 4, verifyIndex: 1, label: 'F4 · Verificación 2 de 2' },
  { id: 'finish', phase: 4, label: 'F4 · Cierre y recolección' }
];
...
function renderConcept() {
  content.innerHTML = `
    <section class="concept-card">
      ...
      <p class="viewer-help">Arrastren el cuerpo para observarlo desde otras posiciones. En este paso solo observen y comparen; todavía no escriban en la guía.</p>
    </section>`;
  addViewer(document.getElementById('concept-body'), 'prism', { x: 0.06, y: -0.16, z: 0.04 }, true);
}

function renderNames() {
  content.innerHTML = `<section class="concept-card"><h2>Conozcan los nombres</h2><p>En este paso, completen la columna <strong>Nombre</strong> de la guía. Los códigos M1–M6 aparecen en el recurso digital.</p><div id="names-grid" class="model-grid"></div><p class="prompt-note">No modifiquen lo que escribieron en la Fase 2.</p></section>`;
...
function renderFamilies() {
  content.innerHTML = `
    <section class="concept-card">
      <h2>Dos familias para esta sesión</h2>
      <p>Los códigos M1–M6 se mantienen para que puedan completar la guía sin regresar a la Fase 2.</p>
      <div class="family-grid">
        <article class="family-column">
          <h3>P — Poliedro</h3>
          <p>Cuerpo que tiene únicamente zonas planas.</p>
        ...
        <article class="family-column">
          ...
          <p>Cuerpo que tiene al menos una zona curva o redondeada.</p>
          <ul class="family-list"><li><strong>M2</strong> · Cono</li><li><strong>M4</strong> · Cilindro</li><li><strong>M6</strong> · Esfera</li></ul>
        </article>
      </div>
      <p class="prompt-note">Completen la columna <strong>Familia</strong> de la guía usando P o CR.</p>
    </section>`;
}
...
function renderQuick() {
  content.innerHTML = `
    <section class="quick-card">
      ...
      <p>Observen cada referencia, respondan las tres preguntas y pulsen <strong>Comprobar</strong>.</p>
...
function renderAuditIntro() {
  content.innerHTML = `
    <section class="message-card">
      <p class="eyebrow">Auditores del museo</p>
      <h2>¿La etiqueta se mantiene o debe corregirse?</h2>
      <p><strong>Trabajen con F4 de la guía.</strong></p>
      <ol>
        <li>Giren el modelo y revisen la etiqueta propuesta.</li>
        <li>Antes de comprobar, completen en la guía la decisión M/C, el resultado del nombre, el resultado de la familia y la razón.</li>
        <li>Seleccionen en el recurso digital la misma decisión y pulsen <strong>«Comprobar decisión»</strong>.</li>
        <li>Después, indiquen en la guía si mantuvieron o cambiaron su decisión.</li>
      </ol>
      <p class="prompt-note"><strong>No modifiquen lo que registraron antes de comprobar.</strong></p>
    </section>`;
}
...
function renderAudit(index) {
  ...
  content.innerHTML = `
    <section class="audit-card">
      <p class="eyebrow">${item.title} · Auditoría de etiqueta</p>
      <h2>Giren el modelo y revisen la etiqueta</h2>
      <div class="audit-layout">
        <div>
          ...
          <p class="viewer-note">Si el modelo no responde al giro, avísenle al docente.</p>
        </div>
        <div>
          ...
          <p class="prompt-note"><strong>Antes de comprobar: completen este caso en la guía.</strong></p>
          <button id="check-audit" class="button" type="button" disabled>Comprobar decisión</button>
  ...
  check.addEventListener('click', () => {
    ...
    feedback.textContent = correct
      ? 'La decisión M/C coincide. Comparen el resultado correcto con lo que registraron y marquen «La mantuvimos». No modifiquen lo escrito antes de comprobar.'
      : 'La decisión M/C no coincide. Comparen el resultado correcto con lo que registraron y marquen «La cambiamos». No modifiquen lo escrito antes de comprobar.';
...
function renderVerify(index) {
  ...
  content.innerHTML = `
    <section class="verify-card">
      <p class="eyebrow">${item.title} · Mantener/Corregir</p>
      <h2>Preparen la tarjeta M/C</h2>
      ${index === 0 ? '<p class="prompt-note"><strong>Mantengan las tarjetas M/C boca abajo hasta que el docente dé la señal.</strong></p>' : ''}
      <p>Cuando el docente dé la señal, levanten <strong>M</strong> si mantienen la etiqueta o <strong>C</strong> si debe corregirse.</p>
      <div id="verify-viewer" class="viewer verify-model" role="img" aria-label="Modelo de la ${item.title}"></div>
      <div class="label-card"><span>Etiqueta propuesta</span><p class="proposed-label">${item.proposed}</p></div>
      <div id="countdown" class="countdown" aria-live="polite">Esperen la señal</div>
      <div class="verify-actions">
        <button id="start-countdown" class="button" type="button" ${verifyState[index] ? 'disabled' : ''}>Iniciar 3–2–1</button>
      </div>
      <div id="verify-feedback" class="feedback" role="status" aria-live="polite"></div>
      <div id="verify-final" class="final-label" ${verifyState[index] ? '' : 'hidden'}><span>Respuesta</span><strong>${item.decision} · ${item.decision === 'M' ? 'Mantener' : 'Corregir'}</strong><small>Etiqueta final: ${item.final}</small></div>
    </section>`;
  ...
  if (verifyState[index]) {
    countdown.textContent = item.decision;
    feedback.className = 'feedback is-success';
    feedback.textContent = index === 0
      ? 'Comparen su tarjeta con la respuesta. Mantengan la tarjeta disponible para la segunda verificación.'
      : 'Comparen su tarjeta con la respuesta. Luego guarden la tarjeta M/C.';
  }
  button.addEventListener('click', async () => {
    ...
    countdown.textContent = item.decision;
    verifyState[index] = true;
    document.getElementById('verify-final').hidden = false;
    feedback.className = 'feedback is-success';
    feedback.textContent = index === 0
      ? 'Comparen su tarjeta con la respuesta. No cambien lo registrado y preparen la tarjeta para la segunda verificación.'
      : 'Comparen su tarjeta con la respuesta. No cambien lo registrado y guarden la tarjeta M/C.';
    nextButton.disabled = false;
  });
}
...
function renderFinish() {
  content.innerHTML = `
    <section class="finish-card">
      <p class="eyebrow">Fin de la Fase 4</p>
      <h2>Revisen la Fase 4</h2>
      <p>Antes de continuar, revisen que los tres casos estén completos y que las respuestas iniciales sigan visibles.</p>
      <p class="closing-note">Entreguen la guía al docente y esperen la indicación para iniciar la Fase 5.</p>
      <p class="finish-badge">✓ Fases 3 y 4 completadas.</p>
    </section>`;
}

function setHeader(screen) {
  const phase3 = screen.phase === 3;
  const isFinish = screen.id === 'finish';
  const isVerify = screen.id === 'verify';

  document.getElementById('phase-lead').textContent =
    isFinish
      ? 'Revisen la Fase 4 y esperen la indicación del docente para continuar.'
      : isVerify
        ? 'Observen el modelo, revisen la etiqueta y preparen la tarjeta M/C.'
        : phase3
          ? 'Observen y comparen. Completen la guía solo cuando el recurso digital lo indique.'
          : 'Giren cada modelo, revisen la etiqueta y decidan si se mantiene o se corrige.';
  document.getElementById('progress-label').textContent = screen.label;
  document.getElementById('progress-state').textContent =
    `Paso ${currentIndex + 1} de ${screens.length}`;
...
function canContinue(screen) {
  ...
  if (screen.id === 'verify') return verifyState[screen.verifyIndex];
  return true;
}
...
function render() {
  ...
  nextButton.textContent = screen.id === 'quick' ? 'Ir a Auditores →' : 'Siguiente →';
