...
function renderModel() {
  ...
  document.getElementById('model-code').textContent = model.code;
  document.getElementById('progress-label').textContent = `${model.code} · Modelo ${currentIndex + 1} de 6`;
  document.getElementById('attempt-label').textContent = modelState.completed ? 'Selección completada' : `Intento ${Math.min(modelState.attempts + 1, 2)} de 2`;
  document.getElementById('progress-bar').style.width = `${((currentIndex + 1) / 6) * 100}%`;
  previousButton.disabled = currentIndex === 0;
  nextButton.disabled = !modelState.completed;
  nextButton.textContent = currentIndex === 5 ? 'Ir a Analiza →' : 'Siguiente →';
  ...
  if (modelState.completed) {
    ...
    feedback.textContent = solvedCorrectly
      ? 'Correcto. Registren en la guía qué observaron (máx. 6 palabras).'
      : 'La opción correcta quedó señalada. Giren el modelo, compárenlo con la opción señalada y registren en la guía qué observaron (máx. 6 palabras).';
  }
  updateDiagnostics();
}
...
function selectOption(index) {
  ...
  if (index === model.correct) {
    ...
    feedback.textContent = 'Correcto. Registren en la guía qué observaron (máx. 6 palabras).';
  } else {
    ...
    if (modelState.attempts >= 2) {
      ...
      feedback.textContent = 'La opción correcta quedó señalada. Giren el modelo, compárenlo con la opción señalada y registren en la guía qué observaron (máx. 6 palabras).';
    } else {
      feedback.className = 'feedback is-warning';
      feedback.textContent = `Esa opción no corresponde. Giren ${model.code} y comparen otra vista.`;
    }
  }
  document.getElementById('attempt-label').textContent = modelState.completed ? 'Selección completada' : `Intento ${modelState.attempts + 1} de 2`;
  nextButton.disabled = !modelState.completed;
  updateDiagnostics();
}
...
function showAnalyze() {
  ...
  document.getElementById('progress-label').textContent = 'Analiza · M4';
  document.getElementById('attempt-label').textContent = 'Comparación guiada';
...
document.getElementById('analyze-form').addEventListener('submit', (event) => {
  ...
  if (!value) { analyzeFeedback.className = 'feedback is-warning'; analyzeFeedback.textContent = 'Seleccionen una afirmación antes de comprobar.'; return; }
  if (value === 'sara') {
    analyzeFeedback.className = 'feedback is-success';
    analyzeFeedback.textContent = 'Sara describe mejor lo que ocurre.';
    ...
    analyzeFeedback.textContent = 'Revisen las dos vistas y vuelvan a comparar. No cambien la respuesta que ya registraron en la guía.';
  }
});
...
document.getElementById('to-oral').addEventListener('click', () => {
  ...
  document.getElementById('attempt-label').textContent = 'Cierre de la fase';
  oralView.scrollIntoView({ block: 'start' });
});

function updateDiagnostics() {
  if (!diagnosticMode) return;
  document.getElementById('diag-model').textContent = models[currentIndex]?.code || 'Analiza';
